...
export const sendOrderEmails = async (order) => {
  console.log(`📧 Triggering order emails with product photos for order ${order.orderId}...`);
  ...
  if (!customerEmail) {
    console.log(`⚠️ No customer email provided for order ${order.orderId}. Only sending admin email.`);
  }
  ...
  try {
    ...
    // Send to Customer (if customer email exists)
    if (customerEmail) {
